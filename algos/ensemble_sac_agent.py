...
Implements the Soft Actor-Critic (SAC) algorithm with Q-fn ensemble
"""
...
EnsembleSacLossInfo = collections.namedtuple(
    'EnsembleSacLossInfo', ('critic_loss', 'actor_loss', 'alpha_loss'))
...
@gin.configurable
class EnsembleSacAgent(tf_agent.TFAgent):
  """A SAC Agent."""
  ...
  def _initialize(self):
    ...
    """
    for cn, tcn in zip(self._critic_networks, self._target_critic_networks):
  ...
  def _train(self, experience, weights):
    ...
    for cn in self._critic_networks:
      trainable_critic_variables += cn.trainable_variables
    ...
    for opt, cn in zip(self._critic_optimizer, self._critic_networks):
      critic_grads = tape.gradient(critic_loss, cn.trainable_variables)
      self._apply_gradients(critic_grads, cn.trainable_variables, opt)
    del tape
    ...
    extra = EnsembleSacLossInfo(critic_loss=critic_loss,
      ...
      def update():
        ...
        for cn, tcn in zip(self._critic_networks, self._target_critic_networks):
          critic_update_1 = common.soft_variables_update(
              cn.variables,
              tcn.variables, tau)
          critic_updates.append(critic_update_1)

        return tf.group(*critic_updates)
